...
func TestCtx(t *testing.T) {
	...
	request := httptest.NewRequest("GET", "/hello?name=Aldi", nil)
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Hello Aldi", string(bytes))
...
func TestHttpRequest(t *testing.T) {
	...
	request.Header.Set("firstname", "Aldi Syah")
	request.AddCookie(&http.Cookie{Name: "lastname", Value: "Putra"})
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Hello Aldi Syah Putra", string(bytes))
}
...
func TestRouteParameter(t *testing.T) {
	...
	request := httptest.NewRequest("GET", "/users/aldi/orders/10", nil)
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Get Order 10 From User aldi", string(bytes))
}
...
func TestFormRequest(t *testing.T) {
	...
	body := strings.NewReader("name=Aldi")
...
func TestRequestBody(t *testing.T) {
	...
	body := strings.NewReader(`{"username":"Aldi", "password":"rahasia"}`)
...
func TestBodyParserJSON(t *testing.T) {
	...
	body := strings.NewReader(`{"username":"Aldi", "password":"rahasia", "name":"Aldi Syah Putra"}`)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Register Success Aldi", string(bytes))
}
...
func TestBodyParserXML(t *testing.T) {
	...
	body := strings.NewReader(
		`<RegisterRequest>
			<username>Aldi</username>
			<password>rahasia</password>
			<name>Aldi Syah Putra</name>
		</RegisterRequest>
		`)
	request := httptest.NewRequest("POST", "/register", body)
...
func TestResponseJSON(t *testing.T) {
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, `{"name":"Aldi Syah Putra","username":"aldi"}`, string(bytes))
}
